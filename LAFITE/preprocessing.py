# AUTOGENERATED! DO NOT EDIT! File to edit: ../02_preprocessing.ipynb.
    ...
    with open(bed_file) as f:
        for read in f:
            chrom, start, end, name, score, strand, thick_start, thick_end, item_rgb, block_count, block_sizes, block_starts = split_bed_line(
                read)  # read the each nanopore read
            full_block = [start+1]
            if block_count > 1:
                for i in range(block_count-1):
                    left_sj = start + block_starts[i] + block_sizes[i]
                    ...
                    if tmp_sj not in junction_dict[(chrom, strand)]:
                        ...
                        if intron_motif in ['GT-AG', 'CT-AC', 'GC-AG', 'CT-GC', 'AT-AC', 'GT-AT']:
                            junction_dict[(chrom, strand)][tmp_sj] = [
                                1, intron_motif, 'canonical']
                        else:
                            junction_dict[(chrom, strand)][tmp_sj] = [
                                1, intron_motif, 'non-canonical']
                    else:
    ...
    if polyadenylation_event:
        ...
        with OPEN(polyadenylation_event,mode=mode) as f:
            ...
            for line in f:
                ...
                if len(line) != 10:
                    raise ValueError(
                        "Fatal: input polyA results was not from nanopolish!")
                elif line[9] == 'PASS':
